...
We can track a loan balance or retirement plan by looping over dates.  We will illustrate for a loan balance.  We will calculate the balance at each date *after* the payment at that date and store the sequence of balances in a list.
...
We can calculate the ending balance for a loan or retirement plan by computing the future value of all cash flows as of the ending date.  We illustrate for a retirement plan.  We allow the deposits to grow at a rate $g$, and we allow the withdrawals to grow (or decline if $h<0$) at a rate $h$.  An easy way to generate a sequence of growing or declining numbers is to use the numpy arange function.
