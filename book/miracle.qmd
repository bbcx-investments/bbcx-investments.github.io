...
## Continuously compounded returns
 ...
 We could consider even more frequent compounding.  In the limit, as compounding becomes continuous, we obtain
$$\lim_{n \rightarrow \infty}  \left(1 + \frac{0.1268}{n}\right)^{n} - 1  = e^{0.1268} - 1 = 0.1352,$$
where $e$ is the natural exponential ($2.718\ldots$).  More generally, [continuous compounding]{style="color: Tomato"} at rate $z$ means that \$1 grows to $e^z$ dollars in a year.  Obviously, continuous compounding is never encountered in practice, but it is a useful "as if" device for some calculations that we will encounter later.
...
r = 0.04
amount = 500000
payment = npf.pmt(rate=r/12, nper=n*12, pv=amount, fv=0)
 
# question 2
r = 0.04
payment = -2000
amount = npf.pv(rate=r/12, nper=n*12, pmt=payment, fv=0)
 
# question 3
amount = 500000
payment = -2000
rate = 12*npf.rate(nper=n*12, pv=amount, pmt=payment, fv=0)
...
$$r = \sqrt{0.75} - 1 = - 0.134.$$
Losing 13.4% two years in a row will put us 25% down.  In general, the geometric average is computed as
$$r = [(1+r_1) \cdots (1+r_n)]^{1/n}.$$
To continue the example, to get back to even after losing 50\%, clearly an asset needs to double, meaning a 100% return.  The geometric average would be zero, and the arithmetic average would be 25%.  The large difference between the two is due to the extreme variability in this example.
...
Thus, "geometric average" means "average in logs."  These logarithms of one-plus-returns are called [continuously compounded returns]{style="color: Tomato"}.  Recall that if \$1 is invested for a year at rate $z$ with continuous compounding, then it grows to $e^z$ dollars, where $e$ is the natural exponential.  So, by investing \$1 for a year at rate $\log (1+r)$ with continuous compounding , at the end of the year we have $e^{\log (1+r)} = 1+r$.  Therefore, continuous compounding at rate $\log (1+r)$ is the same as earning $r$.  The upshot is that "geometric average" means a normal (arithmetic) average for continuously compounded returns.  Returning to the earlier example, suppose an asset drops by 50\% and then has a 100\% return, getting back to even.  The continuously compounded returns are $\log (1-0.5) = \log 0.5$ and $\log (1+1) = \log 2$, and it is indeed true that $\log 0.5 + \log 2 = 0$, so the average continuously compounded return represents the true experience of the asset -- it broke even.
:::

## Retirement planning

Let's put some things together to address an important problem: planning for retirement.  We may already have some funds in an investment account, and we plan to make additional deposits monthly.  At some point, we'll start making withdrawals monthly.  All the while,  the funds in the account earn some rate of return.  For simplicity, we'll assume here that the return is the same each month.  We'll later look at simulating random returns.  We'll also ignore taxes here, deferring that discussion to a later chapter.

We'll use a real monthly rate of return.  Denote it by $r$.  It should be something like a half percent ($r=0.005$) or a third of a percent.  We'll take the future deposits and withdrawals to be in today's dollars.  
...
and if there is anything remaining in the account at that time it earns a final return, producing a "bequest" of
$$B_n = (1+r)B_{n-1}.$$
...
Thus, the inputs are $m$, $n$, $r$, $B_0$, $g$, $D_1$, and $W$.  @fig-retirement presents an example.  See [https://bbcx-investments.com](https://bbcx-investments.com) for an interactive figure with user inputs.

```{python}
#| label: fig-retirement
#| fig-cap: "A retirement account with m=360 month of deposits, n= 720 total months, a monthly rate of return of r= 0.5%, an initial balance of B_0 =  $100,000, a deposit growth rate of g=0.1%, an initial deposit of D_1 =  $1,000, and monthly withdrawals equal to W = $10,000."
...
string = "month %{x}<br>balance=$%{y:,.0f}"
trace = go.Scatter(
    x=np.arange(n+1),
    y=B,
    mode="lines",
...
We can compute the ending balance as the future value of the initial balance plus the future values of the deposits minus the future values of the withdrawals.  
...
This formulation is convenient if we want to solve for an input that makes the plan feasible, given other inputs.  For example, we can solve for the feasible withdrawal as follows:

```p
W = (FVB + np.sum(FVD)) / (1+r)**np.arange(n-m, 0, -1)
```
:::
